...
use rascii::{
    prelude::*,
    search::{
        search_any_sub_command,
        index_any_sub_command,
    },
    command_line::{
        read_env_args,       
        print_and_exit,
...
'/r,   '[/i    [c(?--------?|c]  ^(f+IIi]|0i.     ^?t?'     ^-(1->lI;;;;\"   ]c(.  \"rul
 ...
 /* main errors */
 let path_error = "conside including a file path to allow program to find an image to create art!";
...
} else { // If you type -h || --help only help is going to be displayed!
    let path = index_any_sub_command(&path_codes, &sub_commands).unwrap_or_else(|i| {
        print_and_exit(path_error);
    });
    if let true = search_any_sub_command(&color_codes, &sub_commands) {
        unimplemented!(); // has to create colored image!
    } else { // if no decleration for colored input, else block is gonna run: has to create one colored image;
        let text_color:Vec<u8> = if let Ok(i) = index_any_sub_command(&with_color_codes, &sub_commands) {
            if sub_commands[i..].len() < 4 { // elimination of panic! macro on overindexing!
                print_and_exit(with_color_error);
            } else {
                let rgb = &sub_commands[i + 1_usize..i + 4_usize];
                rgb.iter().map(|v| v.parse::<u8>().unwrap_or_else(|_| {
                    print_and_exit(parse_error);
                })).collect()
            }
        } else {
            vec![255, 255, 255] // If there is no decleration, default color for output text is white.
        };
